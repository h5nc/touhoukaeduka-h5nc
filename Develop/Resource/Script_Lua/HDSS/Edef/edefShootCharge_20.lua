function Edef_ShootChargeEnemyEID_20_1(playerindex, level, x, y, timer, px, py)
	...
	if math.mod(timer, timemod) == 1 then
		...
		hdss.Call(
			HDSS_BL,
			{
				playerindex, num, baseangle, 0, 0, x, y, baseangle, 0, speed, 0.3, type, 4
			}
		)
	end
	return true;
...
function Edef_ShootChargeEnemyEID_20_2(playerindex, level, x, y, timer, px, py)
...
function Edef_ShootChargeEnemyEID_20_3(playerindex, level, x, y, timer, px, py, overbgs)
	if overbgs == nil or overbgs == false then
		_ePlayerShootCharge_3_SetBGS(playerindex, 20);
	end
	if timer == 1 then
	...
	elseif math.mod(timer, 90) == 60 then
		...
		if pattern == 0 then
			...
			hdssEB(CC_ShootChargeEnemyEID_20_3_A, playerindex, x, y, 9000, 0, LConst_EnemyTypeNull, 10);
			hdssENSAIM(playerindex, level, 0, 0, RANDT(1, 36000));	
				
		elseif pattern == 1 then
			local angle = RANDT(-3000, 3000);
			hdssEB(CC_ShootChargeEnemyEID_20_3_B, playerindex, x, y, angle, hdss.Get(HDSS_SIGN, angle), LConst_EnemyTypeNull, 10);
			hdssENSAIM(playerindex, level);	
			
		elseif pattern == 2 then
			
			hdssEB(CC_ShootChargeEnemyEID_20_3_C, playerindex, x, y, 9000, 0, LConst_EnemyTypeNull, 10);
			hdssENSAIM(playerindex, level);	
		
		elseif pattern == 3 then
			
			hdssEB(CC_ShootChargeEnemyEID_20_3_D, playerindex, x, y, 9000, 0, LConst_EnemyTypeNull, 10);
			hdssENSAIM(playerindex, level);	
			
		else
			hdssEB(CC_ShootChargeEnemyEID_20_3_E, playerindex, x, y, 9000, 0, LConst_EnemyTypeNull, 10);
			hdssENSAIM(playerindex, level);	
		end
	end
	return true;
...
function Edef_ShootChargeEnemyEID_20_3_A(playerindex, level, x, y, timer, px, py)
...
function Edef_ShootChargeEnemyEID_20_3_B(playerindex, level, x, y, timer, px, py)
	if _ePlayerShootCharge_CheckBoss(playerindex) then
		return true;
	end
	if timer == 1 then
	...
	elseif math.mod(timer, 3) == 0 then
		...
		hdss.Call(
			HDSS_BL,
			{
				playerindex, num, baseangle+9000, 6, beginindex, x, y, baseangle, 0, speed, -0.2, CC_Bullet_Star, 4
			}
		)
		hdssA(playerindex);
	end
...
function Edef_ShootChargeEnemyEID_20_3_C(playerindex, level, x, y, timer, px, py)
	if _ePlayerShootCharge_CheckBoss(playerindex) then
		return true;
	end
	if timer == 1 then
		hdssEA_FADEOUT(playerindex, 20+level);
	end
...
function Edef_ShootChargeEnemyEID_20_3_D(playerindex, level, x, y, timer, px, py)
	if _ePlayerShootCharge_CheckBoss(playerindex) then
		return true;
	end
	if timer == 1 then
	...
	elseif math.mod(timer, 8) == 6 then
		...
		ePlayerSendExChase_20(playerindex, x, y, playerID, 0, angleoffset);
	end
	return true;
end


function Edef_ShootChargeEnemyEID_20_3_E(playerindex, level, x, y, timer, px, py)
	if _ePlayerShootCharge_CheckBoss(playerindex) then
		return true;
	end
	if timer == 1 then
		local num = math.floor(level/6) + 3;
	...
	elseif math.mod(timer, 14) == 6 then
		...
		local y = CenterH + 16;
		local playerID = 0xff;
		ePlayerSendExChase_20(playerindex, x, y, playerID, 0);
	end
	return true;
end