function Scene_s09(playerindex, timer)
	...
	elseif timer == 1 then
		...
		local colover = global.ARGB(0x40, 0xffffff);
		for i=0, 1 do
			hdss.Call(
				HDSS_BGVALEX,
				{
					playerindex, 1-i, SI_Scene_09_1+i
				},
				{
		...
		hdssBGCOLOR(playerindex, 1, colover, colover, 0, 0);
				
		local coltop = global.ARGB(0xff, 0xbdb0cd);
	...
	elseif math.mod(timer, 16) == 0 then
		...
		for i=0, 1 do
			...
			local edgeoffset = 256;
			hdss.Call(
				HDSS_BGVALEX,
				{
					playerindex, bgindex, SI_Scene_09_3+RANDT(0, 1), global.ARGB(0xff, 0xA0A0A0)
				},
				{
